...
`strategy/sailing-economics-contexts.json` is a similarly bounded timing registry for Sailing entry. Its first context distinguishes Pandemonium's fixed entry rewards from the first modeled fixed purchase, a 15-Sailing, 15,000-coin Skiff. It reports exact current level and coin shortfalls for that one objective. Planks, nails, swamp tar or paste, metal bars, cannonballs, rope, logs, and fabric remain target-specific shipbuilding inputs: without a player-selected component, facility, or activity, the context reports no material shortfall. It does not alter Pandemonium's score or eligibility, require a future purchase, select an upgrade, assume a source, or project variable Sailing income or loot.
