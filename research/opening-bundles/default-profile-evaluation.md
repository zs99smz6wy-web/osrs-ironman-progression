...
- Default new-Ironman objective profile.
- Active attention lane.
- Opening purposes and triggers from `strategy/default-opening-decision-context.json`.
- Bounded content-window catalog only; no route or inventory plan.

## Evaluator result

| Window | Factual state | Timing state | Interpretation |
| --- | --- | --- | --- |
| X Marks the Spot | Needs a spade | `accessible_not_ready` | Shared opening objective. Add the sourced spade acquisition only when the episode is detailed. Keep choice XP unallocated. |
| Natural History Quiz | Eligible | `ideal_candidate` | Strong first-Varrock candidate for fixed Hunter/Slayer XP and 28 Kudos, despite appearing in only one reviewed guide opening. |
...
The evidence currently supports these baseline components for the selected decision horizon:

- shared Lumbridge quest starts and X Marks progression;
- X Marks completion once the episode sources a spade;
- first-Varrock dependency work;
...
## Deferred or triggered branches

| Component | Current treatment | Activation evidence |
| --- | --- | --- |
| Restless Ghost completion | Defer while preserving partial progress | Named Prayer threshold, Morytania chain, or efficient return bundle |
| Chronicle | Cash/transport branch | Observed 300-coin budget plus repeated near-term Champions' Guild teleport value |
| Stronghold cash | Cash branch | Identified purchase shortfall, account-security eligibility, and acceptable traversal plan |
| Kourend staging | Transport/dependency branch | Named Kourend handoff or near-term content window |
