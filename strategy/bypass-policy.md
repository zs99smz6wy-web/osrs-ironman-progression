...
A bypass is a route-changing alternative, not automatically a target.

## Decision rules

- A realised useful drop updates the account state and can defer or remove a conventional acquisition path.
- A potential drop is recommended for intentional farming only when expected acquisition cost is competitive with the displaced path and access is already natural.
